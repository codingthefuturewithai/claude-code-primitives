---
name: devflow:build:security-review
description: Security analysis - branch changes, full repo, or specific files. Use this to scan for security vulnerabilities before creating a PR/MR.
...
## Note for AI Assistants - PARSE ARGUMENTS & CONFIG
...
**Load configuration:**
1. Check for config file (project or global)
2. Extract `issues.backend` (jira, gitlab, none)

Store these values:
- SHOW_DISMISSED: true/false
- ISSUE_KEY: the actual issue key (or empty if no issue specified)
- ISSUES_BACKEND: for creating security tickets if needed
...
**If ISSUE_KEY is provided (e.g., "PROJ-123" or "42"):**
...
3. **"ticket"** - Create issue to track this for later
...
- Append to dismissed issues JSON file
...
- Map severity to priority: CRITICAL→Highest, HIGH→High, MEDIUM→Medium, LOW→Low

[If ISSUES_BACKEND = "gitlab"]:
- Create GitLab issue with `mcp__gitlab__create_issue`
- Add appropriate labels: "security", "[severity]"

[If ISSUES_BACKEND = "none"]:
- Display: "No issue tracker configured. Please track this manually."
- Show the full finding details for copying

- Display ticket key/number to user
- Track as "ticketed" in summary with ticket ID
- Continue to next finding
...
[If count > 0, list each:]
- [TICKET-ID]: [vulnerability type] in [file]
...
To review dismissed issues: `/devflow:build:security-review --show-dismissed [ISSUE_KEY]`
...
- **No issues found OR all issues triaged:**
  ...
  ✅ **Ready to proceed:**
  ```bash
  /devflow:build:complete-issue [ISSUE_KEY]
  ```
...
- **Want to re-run security review:**
  ...
  ```bash
  /devflow:build:security-review [ISSUE_KEY]
  ```
...
- **Re-scan after manual fixes:** `/devflow:build:security-review [ISSUE-KEY]`
